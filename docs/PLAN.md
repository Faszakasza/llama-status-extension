...
- **Active model**: `ctx.model` applied on **every** `session_start` reason (`startup`/`resume`/`new`/`fork`/`reload` — pi re-emits session_start with the model already set and no `model_select` event on these paths) + `model_select` for mid-session switches; provider id form `llama-server=<baseUrl>` (fallback: `ctx.modelRegistry.getProvider(id).baseUrl`).
